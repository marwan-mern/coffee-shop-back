...
export const getmeds =async (req, res) => {
    try {
        const Medecins = await MedsModel.find();
...
export const BuyMixProduct = async (req, res) => {
    const { id } = req.params;
    const { Quantity} = req.body;

    const updatedPost = await MedsModel.findOneAndUpdate({uniqueId:id},{ Quantity:Quantity }, { new: true });
    res.json(updatedPost);
}
...
export const AddToStockAfterRemove = async (req, res) => {
    const { id } = req.params;
    const { Quantity , QuantityOverMonth} = req.body;

    const updatedPost = await MedsModel.findOneAndUpdate({uniqueId:id},{ Quantity:Quantity , QuantityOverMonth:QuantityOverMonth }, { new: true });
    res.json(updatedPost);
}
...
export const getSearch =async (req, res) => {
    ...
    try {
        const Medecins = await MedsModel.find({uniqueId:searchItem });
        res.status(200).json(Medecins)
    } catch (error) {
        res.status(404).json({message: error.message})
    }
